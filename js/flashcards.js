...
H5P.Flashcards = (function ($, Audio) {
  ...
  C.prototype.createCardAudio = function (card) {
    ...
    if (card.audio !== undefined) {
      ...
      audio = new Audio(audioDefaults, that.id);
  ...
  C.prototype.addCard = function (index, $inner) {
    ...
    // that.audios.push(card.audio);
    //   console.log(that.audios[index][0]);
    // for (let i=0; i< card)
    ...
    var $card = $(
      ...
      '</div>')
      .appendTo($inner);
    $card.find('.h5p-flashcard-audio').append(that.createCardAudio(card));
  ...
  C.prototype.showAllAudio = function () {
    var that = this;
    that.$card.find('.h5p-audio-inner')
      .removeClass('hide');
  };
  /**
  ...
  C.prototype.resize = function () {
    ...
    if (this.$inner.width() / parseFloat($("body").css("font-size")) <= 31) {
      that.$container.addClass('h5p-mobile');
    }
    else {
      that.$container.removeClass('h5p-mobile');
    }

    //Find container dimensions needed to encapsule image and text.
    that.$inner.children('.h5p-card').each(function () {
